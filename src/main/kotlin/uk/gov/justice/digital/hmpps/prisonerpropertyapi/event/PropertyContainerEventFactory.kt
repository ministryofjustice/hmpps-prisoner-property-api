 ...
 * sync path and the staff-write path produce a consistent envelope. The two paths differ only in the
...
object PropertyContainerEventFactory {
  ...
  private const val STAFF_DESCRIPTION = "A prisoner property container was changed by a member of staff"
  ...
  /** Event for a staff-initiated change. */
  fun staffEvent(
    eventType: PropertyDomainEventType,
    dpsId: UUID,
    prisonerNumber: String,
    changedFields: List<String>?,
  ): HmppsDomainEvent = event(eventType, dpsId, prisonerNumber, STAFF_DESCRIPTION, changedFields)
