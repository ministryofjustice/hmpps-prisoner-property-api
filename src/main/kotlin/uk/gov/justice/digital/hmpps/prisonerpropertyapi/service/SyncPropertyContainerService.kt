...
class SyncPropertyContainerService(
  ...
  @Transactional
  fun sync(request: SyncPropertyContainerRequest): SyncPropertyContainerResponse = upsert(request, migrating = false)

  /** Initial bulk migration of a NOMIS container. */
  @Transactional
  fun migrate(request: SyncPropertyContainerRequest): SyncPropertyContainerResponse = upsert(request, migrating = true)

  private fun upsert(request: SyncPropertyContainerRequest, migrating: Boolean): SyncPropertyContainerResponse {
  ...
  private fun create(request: SyncPropertyContainerRequest, migrating: Boolean): SyncPropertyContainerResponse {
    ...
    if (!migrating) {
      publish(PropertyDomainEventType.CONTAINER_CREATED, saved.id!!, request.nomisPropertyContainerId, request.prisonerNumber, changedFields = null)
    }
    return SyncPropertyContainerResponse(saved.id!!, request.nomisPropertyContainerId, SyncMappingType.CREATED)
  }

  private fun update(existing: PropertyContainer, request: SyncPropertyContainerRequest, migrating: Boolean): SyncPropertyContainerResponse {
    ...
    if (changed.isNotEmpty()) {
      ...
      if (!migrating) {
        publish(PropertyDomainEventType.CONTAINER_UPDATED, existing.id!!, request.nomisPropertyContainerId, request.prisonerNumber, changed)
      }
    }
    return SyncPropertyContainerResponse(existing.id!!, request.nomisPropertyContainerId, SyncMappingType.UPDATED)
  }

  private fun disposalRequiredEvent(container: PropertyContainer, date: java.time.LocalDate, time: LocalDateTime, user: String) = PropertyEvent(container, PropertyEventType.DISPOSAL_REQUIRED, time, user, eventDate = date)

  private fun disposedEvent(container: PropertyContainer, date: java.time.LocalDate, time: LocalDateTime, user: String) = PropertyEvent(container, PropertyEventType.DISPOSED, time, user, eventDate = date)

  private fun publish(eventType: PropertyDomainEventType, dpsId: UUID, nomisId: Long, prisonerNumber: String, changedFields: List<String>?) {
